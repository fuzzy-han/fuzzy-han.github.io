/**
 * AI Universe Smackdown · Interactive Battle Arena & Animation Engine
 */
...
class SmackdownArena {
  constructor() {
    ...
    this.fighters = {
      gemini: {
        ...
        maxHp: 100,
        skills: [
          { name: '150t/s 光速思维炮', dmg: 26, type: 'laser', roast: '150 tok/s 激光扫射！你还在加载第一句？' },
          { name: 'SWE-bench 反向修Bug斩', dmg: 35, type: 'crit', roast: '70.3% SWE-bench 斩击！一秒修好你的崩溃栈！' },
          { name: '1M 上下文黑洞', dmg: 48, type: 'ult', roast: '把你整座知识库瞬间吸入！毫无延迟！' }
        ]
      },
      openai: {
        id: 'openai',
        ...
        maxHp: 95,
        skills: [
          { name: '40秒草莓沉思', dmg: 18, type: 'delay', roast: '*沉思中...* 等我40秒算个草莓谜题！' },
          { name: '草莓流星轰炸', dmg: 30, type: 'boom', roast: '发射草莓大炮！哎呀消耗了我 8000 thinking tokens！' },
          { name: '高峰期排队暴击', dmg: 42, type: 'ult', roast: '【Capacity Exceeded】排队中，请升级 Plus 账户！' }
        ]
      },
      claude: {
        id: 'claude',
        ...
        maxHp: 95,
        skills: [
          { name: '3000字安全免责声明', dmg: 20, type: 'shield', roast: '首先，我必须以负责任的态度指出：打架是不提倡的...' },
          { name: '哲学长文伦理冲击', dmg: 32, type: 'magic', roast: '以莎士比亚十四行诗格式对你展开伦理说教！' },
          { name: '额度耗尽警告', dmg: 44, type: 'ult', roast: '您在下午5点前还剩2条消息额度，请节约使用！' }
        ]
      },
      deepseek: {
        id: 'deepseek',
        ...
        title: '🐋 671B 开源破壁蓝鲸',
        emoji: '🐋',
        auraColor: 'rgba(96, 165, 250, 0.7)',
        maxHp: 90,
        skills: [
          { name: '开源真香冲击', dmg: 22, type: 'slam', roast: '梁总神算！600万美元训出来的 671B 砸向你！' },
          { name: 'MoE 专家集群冲撞', dmg: 34, type: 'swarm', roast: '启动专家路由！每个专家轮流踢你一脚！' },
          { name: '服务器繁忙 503 绝杀', dmg: 46, type: 'ult', roast: '【503】服务器繁忙，请稍后再试！瞬间冻结战场！' }
        ]
      }
    };
  ...
  resetMatch() {
    if (this.autoMatchInterval) {
      ...
      this.autoMatchInterval = null;
      const autoBtn = document.getElementById('autoBattleBtn');
      if (autoBtn) autoBtn.textContent = '⚔️ 自动大乱斗观战';
    }
    ...
    // Update skill buttons text
    const s1 = document.getElementById('skillBtn1');
    const s2 = document.getElementById('skillBtn2');
    const s3 = document.getElementById('skillBtn3');

    if (s1 && p1.skills[0]) s1.innerHTML = `<span>⚡</span> ${p1.skills[0].name}`;
    if (s2 && p1.skills[1]) s2.innerHTML = `<span>💥</span> ${p1.skills[1].name}`;
    if (s3 && p1.skills[2]) s3.innerHTML = `<span>👑</span> ${p1.skills[2].name}`;

    this.updateHpBars();

    if (this.combatFeed) {
      this.combatFeed.innerHTML = `
        <div class="feed-item highlight">🏆 比赛开始！【${p1.name}】 VS 【${p2.name}】</div>
        <div class="feed-item">准备就绪，点击技能释放超强 AI 招式！</div>
      `;
    }
  }
  ...
  updateHpBars() {
    ...
    if (this.p1HpText) this.p1HpText.textContent = `${this.p1Hp} / ${p1.maxHp} HP`;
    if (this.p2HpText) this.p2HpText.textContent = `${this.p2Hp} / ${p2.maxHp} HP`;
  }
  ...
  usePlayerSkill(skillIdx) {
    ...
    if (window.soundEngine) {
      if (skill.type === 'laser') window.soundEngine.playLaser();
      else if (skill.type === 'ult') window.soundEngine.playExplosion();
    ...
    setTimeout(() => {
      ...
      // P2 Hurt & Damage
      const dmg = skill.dmg + Math.floor(Math.random() * 6);
      this.p2Hp = Math.max(0, this.p2Hp - dmg);
      this.updateHpBars();

      if (this.p2FighterEl) this.p2FighterEl.classList.add('hurt');
      this.showDamageNumber(this.p2FighterEl, `-${dmg}`);

      this.logFeed(`💥 <strong>${p1.name}</strong> 施展【${skill.name}】，对 ${p2.name} 造成 ${dmg} 点暴击伤害！`, 'highlight');
      ...
      setTimeout(() => {
        ...
        // P2 AI Counter-Attack
        setTimeout(() => this.p2CounterAttack(), 600);
      }, 400);
    }, 300);
  }
  ...
  p2CounterAttack() {
    ...
    const p1 = this.fighters[this.p1Current];
    const p2 = this.fighters[this.p2Current];
    const randomSkill = p2.skills[Math.floor(Math.random() * p2.skills.length)];

    if (this.p2FighterEl) this.p2FighterEl.classList.add('attacking');
    this.showRoastBubble(this.p2FighterEl, randomSkill.roast);
    ...
    setTimeout(() => {
      ...
      const dmg = randomSkill.dmg + Math.floor(Math.random() * 5);
      this.p1Hp = Math.max(0, this.p1Hp - dmg);
      this.updateHpBars();

      if (this.p1FighterEl) this.p1FighterEl.classList.add('hurt');
      this.showDamageNumber(this.p1FighterEl, `-${dmg}`);

      this.logFeed(`⚡ <strong>${p2.name}</strong> 反击【${randomSkill.name}】，造成 ${dmg} 点伤害！`, 'crit');
  ...
  handleVictory(winner, loser) {
    ...
    this.showRoastBubble(
      ...
      `🏆 胜利！${winner.name} 绝对优势击溃对手！`
    );
    this.logFeed(`🎉 【KO!】${winner.name} 赢得了本次 AI SmackDown 宇宙冠军！`, 'highlight');

    if (window.showToast) {
      window.showToast(`👑 ${winner.name} 赢得大乱斗胜利！`);
    }
  }
  ...
  toggleAutoBattle() {
    ...
    if (autoBtn) autoBtn.textContent = '⏸️ 暂停观战';
