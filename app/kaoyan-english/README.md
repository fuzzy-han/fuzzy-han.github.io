...
一句「输出被截断」，**拿不到任何报告**，原因是：

批改指令是写给读者看的（「一、整体评价与评分…」「二、逐句批改…」），
思考型模型会**优先照做这套指令**，写出一篇几千字的中文报告，
直到撞上 `max_tokens` 被截断——它压根没打算输出 JSON。
...
- 首轮与重试都带预填充；流式路径也能带（预填充会原样拼回，见 `applyPrefill`）
- 输出额度不足时自动放大到**至少 16384**（一次完整报告实测要 6000–9000 tokens，
  原来翻倍到 8192 常常不够）
- 放大重试改用**非流式**：首次流式都吐不出正文，再流式一次大概率还是空
- 错误文案区分「推理占满额度」与「正文没写完」，引导到正确的排查方向

回归测试：`scripts/verify-json.mjs`（预填充拼回）、
`scripts/verify-reasoning-model.mjs`（真实思考型模型端到端）。

## 批改卡在「正在校验评分」？

如果进度提示停在「正在校验评分… 已接收 0 字符」不动，是 **IndexedDB 写不进去**了。

成因：IDB 存在「库在、但对象仓库丢了」的半损坏状态（其他工具建过同名库、
升级中途失败、浏览器回收数据不完整）。这种状态下 IDB **既不报错也不回调**，
调用方会永久挂起——从界面看就是批改完成了却一直不跳转。

平台现在的三层防护：

1. `openDB()` 检测到缺少对象仓库时，自动升版本重建（启动即自愈）
2. 单次事务超过 8 秒直接放行并告警，绝不让整个批改流程被拖死
3. 「清空全部数据」前先 `closeDB()`——否则 `deleteDatabase` 会被活跃连接
   永久阻塞（这也是个真坑：不关连接就删库，删除永远不会执行）

回归测试见 `scripts/verify-selfheal.mjs`。
