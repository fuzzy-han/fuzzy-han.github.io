...
| **模板库**：从批改记录提炼可复用模板、自动合并累加出现次数、按分类记忆 | ✅ |
| **词汇广场**：5288 个考研词、全局检索（词形/释义/短语/同义词）、相似词串联 | ✅ |
...
**5288 个考研词汇**（KaoYan 三分册 5047 词 + 补充 241 词），数据来自
[kajweb/dict](https://github.com/kajweb/dict) 开源词典。

### 全局检索

一次遍历覆盖四个维度，结果会标注「为什么这个词被搜出来」：
...
| 中文释义 | `合作` | 释义命中 |
| 短语 | `short of` | 短语命中 |
| 同义词 | `insufficient` | 同义词命中 |

实测：搜「合作」带出 team / associate / cooperate / cooperative / collaborate / partner；
搜 `insufficient` 带出 short / minus / scarce / scant。

### 相似词串联

每个词自动串起三类关联，**分开呈现**因为记忆价值不同：

| 类型 | 数据来源 | 为什么单独一类 |
|---|---|---|
| 同义词 | 词典 `syno` 字段（覆盖率 95%） | 写作可替换，但要注意语域与搭配差异 |
| 同根词形 | 词典 `relWord` 字段（覆盖率 82%） | 换词性用，如 short → shortly / shortish |
| 形近词 | 按词干前缀 + 长度接近实时计算 | 最容易混，必须对比记 |

同义词和同根词若也在考研词表内，可以**直接点进去**继续串联；
词表外的也会列出来（标注「词表外」），因为同样有参考价值。

### 数据与体积

原始数据 5.09 MB（未压缩）。构建时做了两件事把传输量压到 **1140 KB gzip**：

1. 裁掉本项目用不到的字段（英释、语音参数），限制例句/短语/同义词组数上限
2. **数组化编码** —— 5288 个词条若每条都写 `{"word":…,"usphone":…}` 这种键名，
   光键名就占约 40% 体积。改成位置数组后 gzip 从 2059 KB 降到 1140 KB，
   **不丢任何信息**，解码在客户端做

数据文件按需加载，不打进主 bundle（主 bundle 仍是 315 KB）；首次加载后
缓存进 IndexedDB，之后离线可用。加载是三级缓存：内存 → IndexedDB → 网络。

重新构建数据（需要网络，源包会缓存到 `.vocab-cache/`）：

```bash
pnpm build:vocab
```

> 实现细节：源 zip 设置了通用标志位 bit 3（data descriptor），
> **本地文件头里的压缩/解压大小都是 0**，真实大小只在末尾的中央目录里。
> 按本地头解析会读到 0 字节并报 `unexpected end of file`。

## 模板库

一次批改的价值不该停在「这篇改完了」。模板库把每次批改里值得留下的东西
沉淀成**离开这篇文章也能用**的模板。

**怎么用**：报告页或记录页点「提炼模板」→ 模型从该次批改的逐句问题、修改句、
表达积累里提炼 3–6 条模板 → 自动入库并归入分类。也可以点「提炼全部记录」批量处理。

**「反复出现」是怎么来的**：入库时做合并去重，同一问题再次出现时**累加出现次数**
而不是新增条目。出现 ≥2 次的会被标为「反复出现」并置顶——出现次数越多，
越该优先记住它。

```
批改记录 → 提炼 → 模板 ─┐
批改记录 → 提炼 → 模板 ─┼→ 同一问题合并，出现次数 +1 → 标记「反复出现」
批改记录 → 提炼 → 模板 ─┘
```

**分类**：句式框架 / 固定搭配 / 衔接表达 / 词汇升级 / 常错点 / 结构模板 / 翻译技巧。
每个分类都带一句「怎么记」的建议，例如固定搭配要求「连着动词一起记」。

**记忆友好设计**：
- 反复出现的置顶并标色，已掌握的自动淡出——注意力留给还没记住的
- 每条模板尽量引用**学生自己的句子**，比范文例句更容易回忆
- 纠错类模板用「原句 → 改后」对照，只记这一组，不抄语法书

### 合并去重的实现细节

相似度用**词集合 Jaccard 与字符 3-gram Jaccard 取较大值**，阈值 0.62。

为什么不能只用词集合：中文没有空格，按空白切词会把整句算成一个 token，
导致「图画作文三段式框架」和「图画作文三段式框架（描述—阐释—评论）」
被判为毫不相干——**这个 bug 是靠真实模型测试发现的**，假模型每次返回同样文案，
永远暴露不了（合并率从 1/5 修复到 3/6）。

确定性算法而非调模型判重：每次提炼都多一次调用不划算，而且措辞相近时
确定性方法已经够用。用户在界面上手动编辑过的模板标记为 `edited`，
不再参与自动合并，避免改动被悄悄覆盖。
